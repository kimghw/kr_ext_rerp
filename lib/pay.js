...
(function (g) {
   ...
   * data-act: pay-toggle(보기/숨기기, 열 때 HR 읽기) · pay-refresh(↻ 모든 달 다시 읽기) · pay-open(＋ 버튼) · pay-edit/pay-del(data-id) · pay-save · pay-cancel, 입력칸 data-pay: name(메모) / amt(금액) */
  function bind(host, state, draw) {
    ...
    /* 구역을 열 때와 ↻ 를 누를 때 그때 HR 급여명세서를 읽는다 (백그라운드 hrCollectNow — 주기 갱신은 없음. 급여는 한 달에 한 번 바뀜).
     * all 이면 모든 달을 다시 읽고, 아니면 목록만 다시 받아 새로 생기거나 소득합계가 바뀐 달만 조회. 결과·오류(로그인 필요 등)는 hrPay.status 로 캐시에 실려 와 다시 그려진다 */
    function collect(all) {
      if (state.payLoading) return;
      state.payLoading = true;
      let p = null;
      try { p = chrome.runtime.sendMessage({ type: 'hrCollectNow', all: !!all }); } catch (e) { p = null; }
      Promise.resolve(p).catch(() => {}).then(() => { state.payLoading = false; draw(); });
    }
    ...
    host.addEventListener('click', (ev) => {
      ...
      if (act === 'pay-toggle') { state.payOpen = !state.payOpen; if (!state.payOpen) close(); else collect(false); draw(); }   // 급여·연구수당 구역 보기/숨기기 (저장하지 않음: 새로 열면 숨김). 열 때 HR 을 읽어 옴
      else if (act === 'pay-refresh') { collect(true); draw(); }   // ↻: 모든 달 다시 읽기
