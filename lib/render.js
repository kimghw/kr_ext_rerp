...
(function (g) {
  ...
  function cardsTable(p) {
    ...
    const info = [];
    if (p.issuedCount != null) info.push(`발급 카드 ${F.money(p.issuedCount)}장 중 이 과제 카드 ${F.money(p.ownCount || 0)}장`);
    if (p.acctKnown === false) info.push('과제 계좌 미확인 (발급 카드 전체를 이 과제 카드로 봄)');
    if (p.rawCount != null) info.push(`사용내역 원본 ${F.money(p.rawCount)}건`);
    ...
    const claim = `<div class="krext-sub"><a href="${F.esc(claimLink(p.prjNo, '', ''))}" target="_blank" rel="noopener">청구서(카드) 화면 열기 →</a> <span class="krext-dim">행을 클릭하면 해당 건이 선택된 상태로 열립니다.</span></div>`;
    return `<table class="krext-cards"><thead><tr><th>카드(뒤8자리)</th><th>사용일시</th><th>가맹점</th><th>승인번호</th><th class="krext-num">사용액</th></tr></thead><tbody>${rows}</tbody></table>${claim}${other}`;
  }
  ...
  function cardsSection(d, s, state) {
    ...
    const head = `<div class="krext-sec-title">카드미청구 <span class="krext-badge${d.totalCount ? ' krext-badge-hot' : ''}" title="고유 거래 기준">${F.money(d.totalCount)}건</span>
    ...
    if (!d.projects.length) {
      const why = d.projectCount === 0 ? '조회된 진행과제가 없습니다.'
        : (d.memberFilter === 'no-id' ? `본인 사번/이름을 아직 확인하지 못해 과제를 표시하지 않습니다. <a href="${F.esc(s.rndUrl || RND_MAIN)}" target="_blank" rel="noopener">R&amp;D ERP</a>를 한 번 열거나 설정에 학번/사번을 입력한 뒤 새로고침하세요.`
        : (d.excludedCount >= d.projectCount ? '모든 과제가 설정에서 제외되어 있습니다.'
        : (d.memberFilter === 'applied' && d.notMemberCount >= d.projectCount - d.excludedCount ? '참여인력에 본인이 포함된 과제가 없습니다. 설정에서 사번을 확인하세요.'
        : '표시할 과제가 없습니다 (0건 과제 숨김).')));
      return `<section class="krext-sec">${head}<div class="krext-msg krext-dim">${why}</div></section>`;
    }
  ...
  function render(host, state) {
    ...
    if (!state.collapsed) {
      ...
      if (d) {
        ...
        html += `<div class="krext-cols">${unapprovedSection(d, s)}${d.loginRequired ? '' : cardsSection(d, s, state)}</div>`;
