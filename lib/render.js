...
(function (g) {
  ...
  function cardView(d, state) {
    ...
    return `<div class="krext-prj-list">${listHead('카드 · 사용자 · 연결 과제', '미청구 · 금액 (원)', 0)}${d.cards.map((card) => cardRow(card, state)).join('')}</div>${sharedBlock(d, state)}`;
  }
    ...
    const list = projects.map((p) => {
      ...
      return `<div class="krext-prj${open ? ' krext-open' : ''}${p.count ? '' : ' krext-zero'}">
        <div class="krext-prj-line">
          ...
          <a class="krext-prj-link" href="${F.esc(projectLink(p.prjNo, '05'))}" target="_blank" rel="noopener" title="R&amp;D ERP 과제정보 › 카드 탭 열기">${ICON_CARD}</a>
        </div>
        ${open ? `<div class="krext-prj-body">${cardsTable(p)}</div>` : ''}
      </div>`;
    }).join('');
    return `<section class="krext-sec">${head}<div class="krext-prj-list">${listHead('책임자 · 과제', '미청구 · 금액 (원)', 2)}${list}</div>${sharedBlock(d, state)}</section>`;
  }
  ...
  function render(host, state) {
    ...
    let html = `<div class="krext-head"${headAct}>
      ...
      <button type="button" class="krext-btn${state.loading ? ' krext-spin' : ''}" data-act="refresh" title="새로고침"${state.loading ? ' disabled' : ''}>${ICON_REFRESH}</button>
    ...
    if (!state.collapsed) {
      ...
      html += `</div><div class="krext-foot"><a href="${F.esc(rnd)}" target="_blank" rel="noopener">R&amp;D ERP 열기 ${ICON_EXT}</a><a href="#" data-act="settings">설정</a>${meta.length ? `<span class="krext-foot-r">${meta.join(' · ')}</span>` : ''}</div>`;
    }
