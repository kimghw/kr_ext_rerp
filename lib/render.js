...
(function (g) {
  ...
  function loginNotice(state) {
    const where = state.mode === 'popup'
      ...
      : 'eClass 화면의 ';
    return `<div class="krext-msg krext-warn">R&amp;D ERP 로그인이 필요합니다. ${where}<b>R&amp;D ERP</b> 메뉴를 클릭해서 로그인해주세요.
      (처음 로그인은 이 메뉴를 거쳐야 세션이 만들어집니다.) 로그인 후 ↻ 새로고침 하세요.</div>`;
  }
  ...
  function headerChips(d, state) {
    if (!d) return '';
    if (d.loginRequired) return `<span class="krext-chip krext-chip-warn" title="eClass의 R&amp;D ERP 메뉴를 클릭해서 로그인해주세요"><span class="krext-chip-l">R&amp;D ERP 로그인 필요</span></span>`;
    ...
    return chip('보완요청', u ? u.supplement : null, 'krext-chip-red', `보완요청 (${src})`)
         + chip('신청', u ? u.apply : null, 'krext-chip-blue', `신청 (${src}) · 클릭: ${APPR_BOX_TITLE}`, { href: approvalBoxLink() })
  ...
  function unapprovedSection(d, s) {
    ...
    if (c) {
      body = stat('보완요청', c.supplement, 'krext-red', link, `보완요청 (${src}) · 클릭: R&D ERP에서 확인`)
           + stat('신청', c.apply, 'krext-blue', approvalBoxLink(), `신청 (${src}) · 클릭: ${APPR_BOX_TITLE}`);
    ...
    return `<section class="krext-sec krext-unappr"><div class="krext-sec-title">미승인내역${note ? `<span class="krext-note">${F.esc(note)}</span>` : ''}${right}</div><div class="krext-stats">${body}</div></section>`;
  }
   ...
   * 줄마다 시작일 기준 계획 반영 합계(계획 기간에는 ERP 계상률 대신 계획 참여율)를, 오늘 계획이 있으면 표 아래 "계획 반영" 합계를 보여준다.
  ...
  const sumTxt = (at) => at.parts.map((p) => `${shortNm(p.prjNm) || p.prjNo} ${rateTxt(p.rate)}${p.planned ? '(계획)' : ''}`).join(' + ');
  /* 참여 계획 줄: 과제 행 밑(계획 태그만) 또는 표에 없는 과제(과제명 함께). 이름 칸에 시작일 기준 계획 반영 합계 */
  function partPlanRow(P, pt, plans, e, other, endDt) {
    const st = P.d8(e.st), at = P.totalAt(pt.items, plans, st || F.ymd(new Date()));
    const tip = [`참여 계획 ${rateTxt(e.rate)} · ${e.st || '?'} ~ ${e.end || '미정'}`, e.prjNm || '',
      `시작일 기준 계획 반영 합계 ${rateTxt(at.total)}${at.total > 100 ? ' (100% 초과)' : ''} = ${sumTxt(at)}`,
      `${F.fmtClock(e.ts)} 입력 · 이 브라우저에만 저장`].filter(Boolean).join(' · ');
    ...
    return `<tr class="krext-part-plan" title="${F.esc(tip)}"><td class="krext-part-nm"><span class="krext-plan-ind">↳</span> <span class="krext-tag krext-tag-sub">계획</span> ${other ? `<span class="krext-plan-nm">${F.esc(shortNm(e.prjNm))}</span> ` : ''}<span class="krext-part-at${overCls(at.total)}" title="시작일 기준 계획 반영 합계 (계획 기간에는 ERP 계상률 대신 계획 참여율로 더함)">시작 시 ${rateTxt(at.total)}</span>${acts}</td>
  ...
  function participationSection(d, s, state) {
    ...
    const cls = total > 100 ? ' krext-bar-over' : (total >= 90 ? ' krext-bar-warn' : '');
    const tip = [`본인 계상률 합계 ${rateTxt(total)} / 100% (참여 과제 ${F.money(pt.items.length)}건, 오늘이 참여기간에 드는 계상률기준 행만)`,
      ...
      pt.useId ? '사번 기준' : '이름 기준', '클릭: 과제별 참여율'].filter(Boolean).join(' · ');
    ...
    if (open) {
      ...
      // 역할은 이름 앞에 책임/참여 태그만 (원래 역할명은 툴팁)
      ...
      const roleTag = (x) => {
        ...
        const s = lead ? '책임' : (/보조/.test(full) ? '보조' : '참여');
        const tip = [full, lead && !/책임/.test(full) ? '과제책임자' : ''].filter(Boolean).join(' · ');
        return `<span class="krext-tag${lead ? '' : ' krext-tag-grey'}" title="${F.esc(tip)}">${s}</span> `;
      };
      ...
      const rows = pt.items.map((x) => {
        ...
        let html = `<tr title="${F.esc(t)}">
          <td class="krext-part-nm">${roleTag(x)}<a href="${F.esc(projectLink(x.prjNo, '03'))}" target="_blank" rel="noopener" title="${F.esc(x.prjNm)} · R&amp;D ERP 과제정보 › 참여인력 탭 열기 (${F.esc(x.prjNo)})">${F.esc(shortNm(x.prjNm))}</a>${plus(x)}</td>
          ...
          <td class="krext-part-term krext-dim krext-mono">${F.esc(shortTerm(x.term))}</td></tr>`;
        if (P) {   // 이 과제의 계획 줄(수정 중이면 폼) + 새 계획 폼
          html += P.listOf(plans, x.prjNo).map((e) => (pe && pe.id === e.id) ? form() : partPlanRow(P, pt, plans, e, false, x.endDt)).join('');
      ...
      if (P) {   // 표에 없는 과제의 계획(과제명 함께) + "다른 과제 참여 계획" ＋ (또는 그 폼)
        extra += P.others(plans, pt.items).map((e) => (pe && pe.id === e.id) ? form() : partPlanRow(P, pt, plans, e, true, '')).join('');
      ...
      // 오늘 계획이 있는 과제가 있으면 합계 밑에 "계획 반영" 합계 (계획 과제는 계획 참여율, 나머지는 ERP 계상률)
      const now = P ? P.totalAt(pt.items, plans, F.ymd(new Date())) : null;
      const planFoot = now && now.parts.some((p) => p.planned)
        ? `<tr class="krext-part-planfoot" title="${F.esc(`오늘 기준 계획 반영 합계 = ${sumTxt(now)}`)}"><td>계획 반영</td><td class="krext-num${overCls(now.total)}">${rateTxt(now.total)}</td><td class="krext-dim">${spare(now.total)}</td></tr>` : '';
    ...
    const note = `계상률기준 · ${d.memberFilter === 'applied' ? '내 참여 과제' : '표시 과제'} ${F.money(pt.items.length)}건${pt.totalExcluded ? ` · 체크제외 ${rateTxt(pt.totalExcluded)}` : ''}${plans.length ? ` · 계획 ${F.money(plans.length)}건` : ''}`;
    return `<section class="krext-sec krext-part">${title(F.esc(note))}${head}${body}</section>`;
  }

  /* ---------- 급여·연구수당 (HR System 급여명세서 → content/hr-pay.js 가 화면에서 읽어 storage.local.hrPay 에 저장, 캐시 d.hrPay 로 전달) ----------
  ...
  const HR_HOME = 'https://hr.krs.co.kr/';
  ...
  const monthLabel = (m) => {
    ...
    const mm = /^\d{4}-(\d{2})-/.exec(String((m && m.date) || '')); return mm ? `${Number(mm[1])}월` : t;
  };
  ...
  function paySummary(d, cfg, state) {
    ...
    return { year, hr, months, latest, base, annual, research, paidMonths, resNm, baseNm, list, missing, payMonths, period, missingNos,
      grade, gradeSource: cfg.grade ? '설정' : (hr.grade ? 'HR 화면에서 자동 감지' : ''), rate, rates, cap, plans, planned, remain, remainAfter };
  }
  ...
  function paySection(d, s, state) {
    ...
    if (!(state && state.payOpen)) {
      return `<section class="krext-sec krext-pay"><button type="button" class="krext-pay-toggle" data-act="pay-toggle" title="HR 급여명세서 기준 연구수당 더 받을 수 있는 금액과 받기 예정 금액 보기">급여·연구수당 보기 ▸</button></section>`;
    }
    const ps = paySummary(d, cfg, state);
    const hide = `<button type="button" class="krext-pay-toggle" data-act="pay-toggle" title="급여·연구수당 숨기기">숨기기</button>`;
    const title = (note) => `<div class="krext-sec-title">급여·연구수당 <span class="krext-note">${note}</span>${hide}</div>`;
    if (!ps || !ps.months.length) {
      const scan = d.hrPay && d.hrPay.scan;
      const scanNote = scan && scan.note ? `<div class="krext-sub krext-err">${F.esc(scan.note)}</div>` : '';
      return `<section class="krext-sec">${title('HR System')}<div class="krext-msg krext-dim">아직 수집된 급여 정보가 없습니다.
        <a href="${F.esc(link)}" target="_blank" rel="noopener">HR System</a>의 보상 › 나의 급여정보 › <b>급여명세서</b>를 한 번 열면 기본연봉과 올해 ${F.esc(cfg.researchItem || '연구수당지급')} 합계가 자동으로 수집됩니다.</div>${scanNote}</section>`;
    }
    ...
    if (ps.cap != null) {
      ...
      body += `<div class="krext-sub krext-dim">직급을 알 수 없어 더 받을 수 있는 금액(월 기본연봉 × 12 × 직급 비율 − 올해 지급)을 계산하지 못했습니다. <a href="#" data-act="settings">설정</a>의 급여·연구수당에서 직급(P1~P4)을 고르세요.</div>`;
    }
    body += plannedBlock(ps, state);
    // 올해 급여 달 중 아직 읽지 못한 달이 있으면 합계가 덜 잡힌 것이라 빨간 글씨로만 알림
    if (ps.missingNos.length) body += `<div class="krext-sub krext-err">${F.esc(`${ps.year}년 ${ps.missingNos.map((n) => n + '월').join('·')} 급여 미수집 — HR 급여명세서에서 그 달을 클릭하면 반영됩니다`)}</div>`;
  ...
  function txRow(c, withPrj, prjNo) {
    ...
    return `<tr class="${cls}"${href ? ` data-act="claim" data-href="${F.esc(href)}" title="클릭: R&amp;D ERP 청구서(카드)에서 이 건 열기"` : ''}>
      <td class="${nm ? '' : 'krext-mono'}" title="${nm ? F.esc(c.tail) + ' · ' : ''}${F.esc(c.cardDiv)} ${F.esc(c.user)}">${F.esc(nm || c.tail)}${withPrj ? '' : shared}</td>
  ...
  function budgetSection(d, s, state) {
    ...
    return `<section class="krext-sec">${head}<div class="krext-prj-list">${list}</div></section>`;
  }
  ...
  function render(host, state) {
    ...
    // (받기 예정 연구수당 입력칸 data-pay, 참여 계획 입력칸 data-part 도 같음)
    const ae = typeof document !== 'undefined' ? document.activeElement : null;
    const attr = ae && ae.dataset ? (ae.dataset.plan ? 'plan' : (ae.dataset.pay ? 'pay' : (ae.dataset.part ? 'part' : ''))) : '';
    const keep = attr && typeof host.contains === 'function' && host.contains(ae) ? { sel: `[data-${attr}="${ae.dataset[attr]}"]`, pos: ae.selectionStart } : null;
    host.innerHTML = html;
