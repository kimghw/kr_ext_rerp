...
(function (g) {
  ...
  function participationSection(d, s, state) {
    ...
    const head = `<a href="#" class="krext-part-head" data-act="prj" data-prj="${PART_KEY}" title="${F.esc(tip)}">
      ...
      <span class="krext-dim krext-part-cnt">${F.money(pt.items.length)}개 과제</span></a>`;
    let body = '';
    if (open) {
      ...
      // 과제명은 앞 6자만 (전체 이름은 툴팁), 역할은 이름 앞에 책임/참여 태그만 (원래 역할명은 툴팁)
      ...
      const rows = pt.items.map((x) => {
        ...
        let html = `<tr title="${F.esc(t)}">
          <td class="krext-part-nm">${roleTag(x)}<a href="${F.esc(projectLink(x.prjNo, '03'))}" target="_blank" rel="noopener" title="${F.esc(x.prjNm)} · R&amp;D ERP 과제정보 › 참여인력 탭 열기 (${F.esc(x.prjNo)})">${F.esc(shortNm(x.prjNm))}</a></td>
      ...
      body = `<table class="krext-cards krext-part-table"><thead><tr><th>과제</th><th class="krext-num">참여율</th><th>기간</th></tr></thead>
        <tbody>${rows || '<tr><td colspan="3" class="krext-dim">본인이 참여인력(계상률기준)에 있는 과제가 없습니다.</td></tr>'}</tbody>
        <tfoot><tr><td>합계</td><td class="krext-num${total > 100 ? ' krext-red' : ''}">${rateTxt(total)}</td><td class="krext-dim">${total > 100 ? '100% 초과' : `여유 ${rateTxt(Math.round((100 - total) * 100) / 100)}`}</td></tr></tfoot></table>`;
    }
    const note = `계상률기준 · ${d.memberFilter === 'applied' ? '내 참여 과제' : '표시 과제'} ${F.money(pt.items.length)}건${pt.totalExcluded ? ` · 체크제외 ${rateTxt(pt.totalExcluded)}` : ''}`;
  ...
  function render(host, state) {
    ...
    // (받기 예정 연구수당 입력칸 data-pay 도 같음)
    const ae = typeof document !== 'undefined' ? document.activeElement : null;
    const attr = ae && ae.dataset ? (ae.dataset.plan ? 'plan' : (ae.dataset.pay ? 'pay' : '')) : '';
