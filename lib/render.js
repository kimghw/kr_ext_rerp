  ...
  function paySection(d, s, state) {
    ...
    if (!ps || !ps.months.length) {
      ...
      return `<section class="krext-sec">${title('HR System')}<div class="krext-msg krext-dim">아직 수집된 급여 정보가 없습니다.
        <a href="${F.esc(link)}" target="_blank" rel="noopener">HR System</a>에 한 번 로그인해 두면 급여명세서의 기본연봉·올해 ${F.esc(cfg.researchItem || '연구수당지급')} 합계와 직급이 자동으로 수집됩니다.</div>${hrStatusNote(hr, link)}${scanNote}</section>`;
    }
    ...
    if (ps.missingNos.length) body += `<div class="krext-sub krext-err">${F.esc(`${ps.year}년 ${ps.missingNos.map((n) => n + '월').join('·')} 급여 미수집 — 다음 갱신 때 다시 읽습니다`)}</div>`;
