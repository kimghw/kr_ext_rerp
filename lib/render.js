...
(function (g) {
  ...
  function runStatus(key, r) {
    ...
    const delBtn = `<button type="button" class="krext-prep-btn krext-prep-del" data-act="prep-run" data-mode="delete" data-key="${k}" title="R&D ERP 결의서에서 이 청구내역(임시저장)을 삭제합니다 — 결의서에 이 건만 있으면 결의서 자체가 삭제됩니다. 삭제되면 거래가 미청구 목록으로 돌아오고 청구 준비(청구종류·청구내역·파일)는 남습니다${r.reqNo ? '' : '. 청구번호가 기록되지 않은 항목은 행을 찾지 못해 탭에서 직접 삭제해야 합니다'}">임시저장 삭제</button>`;
    ...
    else if (r.state === 'applied') html = `<span class="krext-prep-state" title="${msg}">신청됨 ${when}</span>`;
  ...
  function pendingBlock(d, state) {
    ...
    const need = list.filter(({ r }) => r.state !== 'applied').length;
    return { n: need, html: `<div class="krext-pend"><div class="krext-pend-head" title="내역 추가(임시저장)만 된 청구는 결의서를 신청해야 결재로 넘어갑니다. 신청은 결의서에 든 청구내역 전체를 보냅니다">청구서 작성됨 · 신청 대기 <span class="krext-badge${need ? ' krext-badge-soft' : ''}">${F.money(need)}건</span></div>${rows}</div>` };
  }
  ...
  function cardsSection(d, s, state) {
    ...
    if (pend.n) dup += ` · 신청 대기 ${F.money(pend.n)}건`;
