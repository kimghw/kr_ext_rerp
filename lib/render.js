...
(function (g) {
  ...
  function hrStatusNote(hr, link) {
    ...
    if (st.loginRequired) return `<div class="krext-sub krext-err">HR System 로그인이 풀려 급여 정보를 읽지 못했습니다 — <a href="${F.esc(link)}" target="_blank" rel="noopener">HR System</a>에 로그인한 뒤 ↻ 를 누르세요${hr.ts ? ` (${F.esc(F.fmtClock(hr.ts))} 수집분 표시 중)` : ''}</div>`;
  ...
  function paySection(d, s, state) {
    ...
    if (!ps || !ps.months.length) {
      ...
      const msg = loading ? 'HR System 급여명세서를 읽는 중…'
        : `아직 수집된 급여 정보가 없습니다. 급여·연구수당 보기를 누르면 <a href="${F.esc(link)}" target="_blank" rel="noopener">HR System</a> 급여명세서에서 기본연봉·올해 ${F.esc(cfg.researchItem || '연구수당지급')} 합계와 직급을 읽어 옵니다 (HR 에 로그인되어 있어야 함).`;
