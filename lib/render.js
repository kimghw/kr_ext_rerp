...
(function (g) {
  ...
  function stat(label, val, cls, href, title) {
  ...
  function chip(label, val, hotCls, title) {
    ...
    const hot = has && val > 0;
    return `<span class="krext-chip${hot && hotCls ? ' ' + hotCls : ''}" title="${F.esc(title || label)}"><span class="krext-chip-l">${F.esc(label)}</span><span class="krext-chip-v">${has ? F.money(val) : '-'}</span></span>`;
  }
  ...
  function headerChips(d) {
    ...
    return chip('보완요청', u ? u.supplement : null, 'krext-chip-red', `보완요청 (${src})`)
         ...
         + chip('카드미청구', d.totalCount, '', `카드미청구 ${F.money(d.totalCount)}건 · ${F.money(d.totalAmount)}원`);   // 건수가 있어도 다른 칩과 같은 회색
  }
  ...
  function unapprovedSection(d, s) {
    ...
    if (d.unapproved) {
      ...
      body = stat('보완요청', d.unapproved.supplement, 'krext-red', link) + stat('신청', d.unapproved.apply, 'krext-blue', approvalBoxLink(), APPR_BOX_TITLE);
      const extra = [];
      ...
      body = stat('보완요청', it['보완요청'] ? it['보완요청'].count : null, 'krext-red', link)
           + stat('신청', it['신청'] ? it['신청'].count : null, 'krext-blue', approvalBoxLink(), APPR_BOX_TITLE);
      const extra = [];
  ...
  function budgetTable(p) {
    ...
    // 자금현황 링크는 과제 행의 📊 아이콘이 담당하므로 여기서는 계산 요약만
    const summary = `<div class="krext-sub krext-dim">${F.esc(notes)}</div>`;
    return `<table class="krext-cards krext-bgt"><thead><tr><th>비목</th><th class="krext-num" title="예산액(A)">예산</th><th class="krext-num" title="예산잔액(A-B)">예산잔액</th><th class="krext-num" title="승인액(B) ÷ 예산액(A)">집행비율</th></tr></thead><tbody>${rows}${ubRow}</tbody><tfoot>${foot}</tfoot></table>${summary}`;
  }
