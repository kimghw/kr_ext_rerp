...
(function (g) {
  ...
  function hrStatusNote(hr, link) {
    ...
    if (st.loginRequired) return `<div class="krext-sub krext-err">HR System 로그인이 풀려 급여 정보를 갱신하지 못했습니다 — <a href="${F.esc(link)}" target="_blank" rel="noopener">HR System</a>에 로그인하면 자동으로 다시 수집합니다${hr.ts ? ` (${F.esc(F.fmtClock(hr.ts))} 수집분 표시 중)` : ''}</div>`;
  ...
  function paySection(d, s, state) {
    ...
    if (!ps || !ps.months.length) {
      ...
      return `<section class="krext-sec">${title('HR System')}<div class="krext-msg krext-dim">아직 수집된 급여 정보가 없습니다.
        <a href="${F.esc(link)}" target="_blank" rel="noopener">HR System</a>에 로그인한 탭을 열어 두면(어느 화면이든) 급여명세서의 기본연봉·올해 ${F.esc(cfg.researchItem || '연구수당지급')} 합계와 직급이 자동으로 수집됩니다.</div>${hrStatusNote(hr, link)}${scanNote}</section>`;
    }
    ...
    if (ps.missingNos.length) body += `<div class="krext-sub krext-err">${F.esc(`${ps.year}년 ${ps.missingNos.map((n) => n + '월').join('·')} 급여 미수집 — HR System 탭을 열면 다시 수집합니다`)}</div>`;
    const scan = ps.hr.scan;
    if (scan && scan.note) body += `<div class="krext-sub krext-err">${F.esc(scan.note)}</div>`;
    body += hrStatusNote(ps.hr, link);
