...
(function (g) {
  ...
  function hrStatusNote(hr, link) {
    ...
    if (st.loginRequired) {
      const how = st.tabOpened
        ? `HR System 탭을 열었지만 자동 로그인이 되지 않았습니다. 그 탭에서 로그인한 뒤`
        : `<a href="${F.esc(link)}" target="_blank" rel="noopener">HR System</a>에 로그인한 뒤`;
   ...
   * "청구서 작성"은 백그라운드 탭에서 R&D ERP 청구서를 열어 세목·청구종류·청구내역·첨부를 넣고 "내역 추가"까지 누른다 (background.js prepRun). 상태는 entry.run */
  ...
  const prepToggle = (state, gkey) => prepCtx.on ? `<a href="#" class="krext-prj-link krext-prj-claim${prepShown(state, gkey) ? ' krext-on' : ''}" data-act="prep-toggle" data-key="${F.esc(gkey)}" title="${prepShown(state, gkey) ? '청구 준비 숨기기' : '청구 준비 보기 — 거래 행 밑에 청구종류 선택 · 청구내역(적요) 입력 · 첨부 파일 놓기 · 청구서 작성 버튼 줄을 보입니다'}">${ICON_CLAIM}</a>` : '';
  const PREP_TIP = '청구 준비: 여기서 고른 청구종류·적은 청구내역과 끌어다 놓은 파일은 이 브라우저에만 저장됩니다. 거래를 클릭해 청구서를 열면 폼에 적용되고, 청구서 작성을 누르면 백그라운드에서 내역 추가까지 합니다';
  /* 조작 요소 묶음: [청구종류 ▾] [청구내역(적요) 입력란] [파일 칩 …] [끌어다 놓기] 상태 [청구서 작성] 지움.
  ...
  function prepControls(key, meta, state) {
    ...
    let run = '';
    let r = e && e.run;
    if (r && r.state === 'running' && Date.now() - (r.ts || 0) > 5 * 60000) r = Object.assign({}, r, { state: 'failed', msg: '제한 시간 안에 결과가 없습니다 — 탭에서 확인하세요' });   // 백그라운드가 꺼져 상태가 안 바뀐 경우
    if (r && r.state === 'running') run = `<span class="krext-prep-state krext-prep-running" title="백그라운드 탭에서 청구서를 작성하는 중 (${F.esc(F.fmtClock(r.ts))} 시작)">작성 중…</span><button type="button" class="krext-prep-btn" data-act="prep-tab" data-key="${F.esc(key)}" title="작성 중인 R&D ERP 탭 보기">탭 보기</button>`;
    else if (r && r.state === 'done') run = `<span class="krext-prep-state" title="${F.esc([r.msg || '', '결재요청은 R&D ERP 에서'].filter(Boolean).join(' · '))}">청구내역 추가됨 ${F.esc(F.fmtClock(r.ts))} <span class="krext-dim">(결재요청은 R&amp;D ERP 에서)</span></span>`;
    else if (r && r.state === 'failed') run = `<span class="krext-prep-state krext-err" title="${F.esc(r.msg || '')}">작성 실패: ${F.esc(r.msg || '')}</span><button type="button" class="krext-prep-btn" data-act="prep-tab" data-key="${F.esc(key)}" title="남겨 둔 R&D ERP 탭에서 이어서 하기">탭 보기</button>`;
    const canRun = !!(e && type) && !(r && r.state === 'running') && !(state.prepBusy && state.prepBusy.key === key);
    const runBtn = e ? `<button type="button" class="krext-prep-btn krext-prep-run" data-act="prep-run" data-key="${F.esc(key)}"${canRun ? '' : ' disabled'} title="${type ? 'R&D ERP 청구서(카드)를 백그라운드 탭으로 열어 이 거래를 고르고 세목·청구종류·청구내역·첨부를 넣은 뒤 “내역 추가”까지 누릅니다 (결재요청은 하지 않음). 실패하면 그 탭을 남겨 둡니다' : '청구종류를 먼저 고르세요'}">${r && r.state === 'failed' ? '다시 작성' : '청구서 작성'}</button>` : '';
    ...
    return `<select class="krext-prep-type${type ? ' krext-on' : ''}" data-prep-type="${F.esc(key)}" title="청구종류 — R&D ERP 청구서의 세목 빠른 선택 항목 (설정의 청구서 입력 도우미에서 변경)">${opts}</select>${ptclBox}
      <span class="krext-prep-files">${chips}${drop}</span>${busy}${note}${attached}${run}${runBtn}${clear}`;
  }
  ...
  function cardsSection(d, s, state) {
    ...
    const prepN = prepCtx.on && state.prep ? Object.values(state.prep).filter((e) => e && !e.missingSince).length : 0;   // 청구 준비(청구종류·첨부) 항목 수 (미청구 목록에서 사라진 것 제외)
    ...
    return `<section class="krext-sec">${head}<div class="krext-prj-list">${listHead('책임자 · 과제', '미청구 · 금액 (원)', prepCtx.on ? 3 : 2)}${list}</div>${sharedBlock(d, state)}</section>`;
  }
