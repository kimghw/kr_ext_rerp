...
(function (g) {
  ...
  async function checkMembership(projects, myIds, myNames, settings) {
    ...
    const key = 'v2#' + myIds.join('|') + '#' + myNames.join('|');   // v2: hits 목록 저장 (이전 형식은 무시)
    const now = Date.now();
    const result = {};
    const todo = [];
    ...
    await pool(todo, 4, async (p) => {
      try {
        ...
        for (const x of list) {
          ...
          const k = x.empNo + '|' + x.empNm;
          if (seen.has(k)) continue;
          seen.add(k);
          hits.push({ empNo: x.empNo, empNm: x.empNm, role: x.role, byId, byName });
        }
        result[p.prjNo] = hits;
  ...
  async function collect(settings) {
    ...
    if (settings.onlyMyProjects) {
      ...
      const myNames = [settings.myName, rndUser && rndUser.userNm].map((x) => String(x || '').trim()).filter(Boolean);
