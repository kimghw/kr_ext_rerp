...
(function (g) {
  ...
  async function checkMembership(projects, myIds, myNames, settings) {
    ...
    const key = 'v4#' + myIds.join('|') + '#' + myNames.join('|');   // v4: 행마다 계상률·기간·종류(구분/소득구분/지급비목)·원본 필드 포함 (이전 형식은 무시)
    const now = Date.now();
    const result = {};
    const todo = [];
    for (const p of projects) {
      const c = cache[p.prjNo];
      if (c && c.key === key && now - c.ts < MEMBER_TTL && Array.isArray(c.hits)) result[p.prjNo] = c.hits; else todo.push(p);
    }
    await pool(todo, 4, async (p) => {
      try {
        ...
        for (const x of list) {
          ...
          const k = [x.empNo, x.empNm, x.src, x.term, x.rate].join('|');   // 같은 사람의 계상률기준/인력기준 행, 참여기간이 다른 행은 각각 남김 (참여율 계산용)
  ...
  async function collect(settings) {
    ...
    const out = {
      ...
      issued: [], projectList: [], myIds: [], myNames: [], memberFilter: 'off', notMemberCount: 0, nameCandidates: [],
      ...
      settings: {
        ...
        quickPicks: (settings.claimHelper && settings.claimHelper.quickPicks) || [],   // 예상 비용 세목 자동완성 (청구서 세목 빠른 선택 표시이름)
    ...
    out.myIds = Array.from(new Set([settings.myEmpNo, rndUser && rndUser.empNo, rndUser && rndUser.userId].map((x) => String(x || '').trim()).filter(Boolean)));
    out.myNames = Array.from(new Set([settings.myName, rndUser && rndUser.userNm].map((x) => String(x || '').trim()).filter(Boolean)));
    ...
    if (settings.onlyMyProjects) {
      if (!out.myIds.length && !out.myNames.length) {
        ...
        try {
          const hits = await checkMembership(projects.slice(0, 200), out.myIds, out.myNames, settings);
    ...
    // 꺼져 있으면 표시할 과제만 참여인력을 조회한다 (12시간 캐시). 사번/이름을 모르거나 동명이인이면 계산하지 않음
    try {
      if (!partHits && (out.myIds.length || out.myNames.length) && projects.length && out.memberFilter !== 'ambiguous') {
        partHits = await checkMembership(projects, out.myIds, out.myNames, settings);
  ...
  g.KRX_API = { call, collect, diagnose, fetchProjects, fetchCards, fetchIssuedCards, fetchAccounts, fetchParticipants, fetchBudget, fetchUnapproved, describe, summarizeParticipation, termRange };
})(self);
