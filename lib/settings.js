...
(function (g) {
  const DEFAULTS = {
    ...
    hr: {                             // HR System(hr.krs.co.kr) 급여명세서 수집 → 패널 "급여·연구수당" 구역 (content/hr-pay.js)
      enabled: true,                  // 끄면 수집도 표시도 하지 않음
      autoScan: true,                 // 급여명세서를 열면 올해 각 달을 자동으로 클릭해 지급내역을 모아 옴 (끄면 직접 클릭한 달만 반영)
      url: 'https://hr.krs.co.kr/',   // 패널 링크
      baseItem: '기본연봉',            // 지급내역(소득명)에서 기본연봉 항목명
      researchItem: '연구수당지급',     // 지급내역(소득명)에서 연구수당 항목명 (올해 1월부터 합계)
      grade: '',                      // 직급 (P1~P4). 비우면 HR 화면에서 자동 감지한 값 사용
