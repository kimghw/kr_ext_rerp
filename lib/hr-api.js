...
(function (g) {
  ...
  const HOME = 'https://hr.krs.co.kr';
  /* SSO 진입: eClass 홈 메뉴 HR › Main Page 가 여는 중계 페이지(제목 "SSO Redirect", eClass 세션 필요, GET). 인라인 JS 가 SSO_PROC_RE 주소(hr.krs.co.kr/sso-proc?UID=…&SID=…)로
   * 이동하면 HR 세션이 만들어진다. HR 루트(HOME/)를 직접 열면 로그아웃 상태에서는 SSO 없이 본문 없는 403 이라 로그인이 안 된다 (2026-09-25 확인). background.js hrAutoLogin 이 쓴다 */
  const SSO_URL = 'https://eclass.krs.co.kr/eClassVer4/External/SSOMessage';
  const SSO_PROC_RE = /https:\/\/hr\.krs\.co\.kr\/sso-proc\?[^'"\s<>]+/i;
  const isSsoBridge = (url) => /^https:\/\/eclass\.krs\.co\.kr\/.*SSOMessage/i.test(String(url || ''));
  /* base: HR 탭 안(같은 출처)에서는 '' (상대 경로), 백그라운드에서는 HOME (절대 경로 + 쿠키 포함; background.js 의 DNR 규칙이 Origin/Referer 를 HR 로 바꿔 준다) */
  let base = '';
  function setBase(b) { base = String(b || '').replace(/\/+$/, ''); }
  ...
  async function getJson(path, params) {
    ...
    if (!/json/i.test(ct) || !text) {
      ...
      // 확장 출처에서 Origin 을 그대로 보내면 서버 CORS 필터가 본문 없는 200 을 준다 → 호출 쪽에서 HR 탭 경로로 대체
      if (res.status === 200 && !text) throw err('empty', 'HR 서버가 빈 응답을 돌려줌 (다른 출처의 요청으로 취급됨)');
      throw err('http', `HTTP ${res.status} (${ct || 'content-type 없음'})`);
    }
    let data;
  ...
  async function collect(opts) {
    ...
    const known = o.known || {};
    const patch = { ts: Date.now(), source: 'api', page: o.page || (typeof location !== 'undefined' && location.pathname) || '', empNo, lists: [], months: [] };
  ...
  g.KRX_HR_API = { SSO_URL, SSO_PROC_RE, isSsoBridge, API, HOME, EMP_RE, setBase, getJson, fetchEmployee, fetchList, fetchIncome, collect };
