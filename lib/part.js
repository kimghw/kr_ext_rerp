 ...
 * 계획이 없는 과제의 그날 참여율(erpRateAt) = 그날 유효한 ERP 계상률 행(다음 달·다음 연차 행이 이미 있으면 그 값) → 없으면 과제 종료일이 지났으면 0(종료 과제) → 아직 진행 중이면 지금 계상률(변경 전) 그대로
 * 어떤 날의 계획 반영 합계(totalAt) = Σ 표의 과제별(그날 계획이 있으면 계획 참여율, 없으면 erpRateAt) + Σ 그날 유효한 표 밖 과제 계획
 * 기간별 계획 반영 합계(periods) = 오늘부터 계획·계상률 행의 시작일, 종료·과제 종료 다음 날을 경계로 잘라, 계획이 든 구간마다 그 첫날의 계획 반영 합계 (합계·구성이 같은 이웃 구간은 하나로)
...
(function (g) {
  ...
  /* 계획이 없는 과제의 그날 참여율 { rate, ended }:
   *  - 그날 유효한 ERP 계상률기준 행(summarizeParticipation 의 item.rows; 연구수당 성격·체크제외 행 제외)이 있으면 종류별로 시작이 가장 늦은 행 하나씩 합
   *    (rnd-api summarizeParticipation 의 "현재" 행 규칙을 임의의 날짜로 — 다음 달·다음 연차 행이 이미 있으면 그 값)
   *  - 없고 과제 종료일(item.endDt)이 지났으면 0, ended (종료 과제는 합계에서 빠짐)
   *  - 없지만 과제가 아직 진행 중이면(다음 연차 계상률이 아직 안 올라옴) 지금 계상률(item.rate, 변경 전)이 이어진다고 본다 */
  function erpRateAt(item, day) {
    ...
    if (rows.length) {
      ...
      return { rate: r2(s), ended: false };
    }
    const end = d8(item && item.endDt);
    return (end && day > end) ? { rate: 0, ended: true } : { rate: r2(F.num(item && item.rate)), ended: false };
  }
  /* 그날의 계획 반영 참여율 { total, parts: [{ prjNo, prjNm, rate, planned, ended }] } */
  function totalAt(items, plans, day) {
    const parts = [];
    for (const x of items || []) {
      const p = pickPlan(listOf(plans, x.prjNo), day), e = p ? null : erpRateAt(x, day);
      parts.push({ prjNo: x.prjNo, prjNm: x.prjNm, rate: p ? rateOf(p) : e.rate, planned: !!p, ended: !p && e.ended });
    }
    const grp = new Map();
  ...
  /* 날짜 더하기 (yyyyMMdd → yyyyMMdd). 말일을 넘는 날(예 'YYYYMM31')은 그 달 말일로 보고 더한다 */
  const addDays = (day, n) => { const y = +day.slice(0, 4), m = +day.slice(4, 6), dd = Math.min(+day.slice(6, 8), new Date(y, m, 0).getDate()); return F.ymd(new Date(y, m - 1, dd + n)); };
  /* 오늘부터의 기간별 계획 반영 합계 [{ st, end('' = 미정), total, parts }] (ERP 합계 줄은 그대로 두고 그 밑에 보여 주는 "변경 후" 값).
   * 계획·계상률 행의 시작일과 종료·과제 종료 다음 날을 경계로 잘라 구간마다 첫날의 totalAt 을 구하고, 계획이 든 구간만 남긴다 (합계·구성이 같은 이웃 구간은 하나로).
   * 마지막 계획이 끝나는 날까지만 보고, 끝이 미정인 계획이 있으면 끝까지(마지막 구간 end '') */
  function periods(items, plans, today) {
    const day0 = today || today8();
    const live = (plans || []).filter((e) => d8(e.st) && (!d8(e.end) || d8(e.end) >= day0));
    if (!live.length) return [];
    const last = live.some((e) => !d8(e.end)) ? '' : live.reduce((m, e) => d8(e.end) > m ? d8(e.end) : m, '');
    const cuts = new Set([day0]);
    const cut = (st, end) => { const s = d8(st), n = d8(end); if (s && s > day0) cuts.add(s); if (n && n >= day0) cuts.add(addDays(n, 1)); };
    for (const e of live) cut(e.st, e.end);
    for (const x of items || []) {
      cut('', x.endDt);   // 과제 종료 다음 날부터 그 과제는 0
      for (const r of (x.rows || [])) if (r && r.src === 'info' && r.rate != null && !r.allowance && !r.rateExcluded) cut(r.st, r.end);
    }
    const pts = Array.from(cuts).filter((c) => !last || c <= last).sort();
    const out = [];
    for (let i = 0; i < pts.length; i++) {
      const st = pts[i], next = pts[i + 1] || '';
      const end = next ? addDays(next, -1) : last;
      const at = totalAt(items, plans, st);
      if (!at.parts.some((p) => p.planned)) continue;
      const sig = at.parts.map((p) => `${p.prjNo || p.prjNm}:${p.rate}:${p.planned ? 1 : 0}`).join('|');
      const prev = out[out.length - 1];
      if (prev && prev.sig === sig && prev.end && addDays(prev.end, 1) === st) prev.end = end;
      else out.push({ st, end, total: at.total, parts: at.parts, sig });
    }
    return out;
  }
  ...
  Object.assign(api, { KEY, PART_KEY, load, save, bind, listOf, others, pickPlan, erpRateAt, totalAt, periods, addDays, monthGrid, addMonth, covers, d8, iso, presetEnd });
  g.KRX_PART = api;
