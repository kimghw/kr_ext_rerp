 ...
 *  정리: 거래가 미청구 목록에서 사라지면(청구 완료 등) missingSince 를 찍고 7일 뒤 지운다 (cleanup, background refresh 마다). 메타에 없는 고아 Blob 도 10분 지나면 지움 */
const KRX_PREP_STORE = (() => {
  ...
  async function setType(key, meta, type) {
    ...
    e.ts = Date.now();
    if (e.run && e.run.state !== 'running') e.run = null;   // 내용이 바뀌면 지난 작성 결과 표시는 지움
  ...
  async function setPtcl(key, meta, text) {
    ...
    e.ts = Date.now();
    if (e.run && e.run.state !== 'running') e.run = null;
    if (isEmpty(e)) delete m[key];
    await saveMeta(m);
    return m[key] || null;
  }
  /* 백그라운드 탭 자동 작성("청구서 작성") 상태: { state: running | done | failed, ts, tabId, msg } */
  ...
  /* currentKeys: 지금 미청구 목록에 있는 거래 키. 없는 항목은 missingSince 를 찍고 7일 지나면 삭제. 메타에 없는 고아 Blob 은 10분 지나면 삭제 */
